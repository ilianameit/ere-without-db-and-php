...
var local = 	{
			"basket_is_empty" : "Нет выбранных работ", 
			"name" : "Название",
			"price" : "Цена",
			"work" : "Вид работы",			
			"all" : "Всего",
			"order" : "Оформить заказ",
			"basket" : "Выбратнные работы",
 ...
 **********************************************************************************************/
	this.addToCart = function(curObj, id, params)
		...
		var goodieLine = {"id" : id_, "name" : params.name, "price": params.price, "work": params.work, "num" : kol, "url" : document.location.href, "photo" : ""};
	...
	this.renderBasketTable = function()
		...
		if ($('#bcontainer').length == 0)
			...
			$("body").append(" \
				...
				<table id='bcaption'><tr><td>ID</td><td>" + local.name + "</td><td>" + local.price + "</td><td>" + local.work + "</td><td>" + local.all + "</td><td></td></tr></table> \
		...
		for(var idkey in this.DATA) 
			...
			with (this.DATA[idkey])
				...
				var productLine = '<tr class="bitem" id="wigoodline-' + id + '"> \
													...
													<td id="lineprice_' + id + '"class="wigoodprice">' + work + ' </td> \
